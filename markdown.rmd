...
md.pattern(dane)
dane_imp <- mice(dane, m = 10, maxit = 10, seed = 123)
dane1 <- complete(dane_imp, action="long")
md.pattern(dane1)
...
4. Czy wiek wpływa na ryzyko zachorowania na raka płuc?
```{r}
...
```{r, include = FALSE}
    ...
    table(dane1$heavy_smoker_no_symptoms, useNA = "ifany")
    ...
    table(dane1$alcohol_healthy_and_fit, useNA = "ifany")
    ...
    table(dane1$snoring_no_risk_factors, useNA = "ifany")
    ...
    table(dane1$fatigue_no_other_symptoms, useNA = "ifany")
    ...
    table(dane1$high_genetic_no_context, useNA = "ifany")
  ...
  table(dane1$suspicious_score)

  dane1 %>% 
  arrange(desc(suspicious_score)) %>% 
  head(10)

  summary_suspicious <- dane1 %>%
  mutate(
...
mean_smoking <- dane1 %>%
  group_by(Level) %>%
  summarise(mean_smoke = mean(Smoking))

barplot(mean_smoking$mean_smoke,
        names.arg = mean_smoking$Level,
        col="skyblue",
        main="Średni poziom palenia vs Level",
        ylab="Średnia")

```
```{r}
barplot(table(dane1$Level),
        col=c("lightgreen","orange","red"),
        main="Liczba pacjentów wg Level",
        ylab="Liczba",
        xlab="Level")

```
```{r}
ggplot(dane1, aes(x = Level, fill = as.factor(Gender))) +
  geom_bar(position="dodge") +
  theme_minimal() +
  ggtitle("Częstość Level wg płci") +
  labs(fill="Płeć")

```
```{r}
ggplot(dane1, aes(x = as.factor(Gender), y = Age, fill = as.factor(Gender))) +
  geom_boxplot() +
  theme_minimal() +
  ggtitle("Wiek wg płci") +
  labs(x = "Płeć")
